...
export const MAP_DIR = '_машина/карта';        // лёгкая модель + словарь для приложения
...
export const CHRONICLE_ZONES = [/^brain\/log$/i, /^_tools\/queue$/i, /^Учёба\/Лог сессий$/i, /^archive$/i];
export const CHRONICLE_COLOR = '#6f7d84';

/* Слои корпуса — классы памяти. Приложение по умолчанию показывает память, а не
   всё подряд: сущности и утверждения. Выключены два класса.

   «Выводимое» — карточки SAT, шаблоны, машинные сводки: 455 объектов из 680,
   которые растут сами и к знанию отношения не имеют.

   «Улика» — журналы, дневники, выгрузки: они не знание, а доказательства, и
   место им в Обсидиане, где ведётся архив. Здесь они всплывают, только когда
   человек сам их включит или пойдёт по ссылке «источник» проверять утверждение.

   `card` остаётся для вальтов без классов. */
export const HIDDEN_LAYERS = ['выводимое', 'улика', 'card'];

/* Папки, которые в карте есть, но по умолчанию не показываются: шаблоны — это
   не память, а заготовки, и на графе они висят одинокими точками. Архив память,
   но прошлая: приглушаем как хроники, а не прячем. */
export const HIDDEN_ZONES = ['templates', 'Учёба/Карточки'];
