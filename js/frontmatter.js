...
/* Правка фронтматтера обязана оставлять тело нетронутым.

   Раньше здесь стояла нормализация: тело обрезалось по началу и приклеивалось
   через одну пустую строку. Выглядит безобидно, а на деле любая простановка
   тега меняла файл за пределами фронтматтера — на карточках словаря, где тела
   почти нет, каждая правка добавляла перевод строки. Тест кругового прогона по
   вальту нашёл 569 таких файлов.

   Теперь хвост берётся ровно тем, чем был: регулярка уже съела перевод строки
   после закрывающего `---`, значит остаток — это тело как есть, вместе с пустой
   строкой или без неё. */
export function applyFm(text, fm, order) {
  const parsed = parseFm(text);
  const head = stringifyFm(fm, order.length ? order : parsed.order);
  const m = text.match(FM_RE);
  // Файла без фронтматтера это тоже касается: ему шапку добавляют впервые, и
  // единственное разумное — отделить её от текста одной пустой строкой.
  const tail = m ? text.slice(m[0].length) : `\n${text.replace(/^\s+/, '')}`;
  return `---\n${head}\n---\n${tail}`;
}
