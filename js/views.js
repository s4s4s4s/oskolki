...
import { $, el, escA, norm2, zn, zoneDot, noteHref, openNote, toast, wireWikiLinks, diffHtml } from './ui.js';

// Реэкспорт: примитивы переехали в ./ui.js, но main.js и десктоп продолжают
// брать их отсюда — менять чужие импорты ради переезда незачем.
export { $, el, toast, zoneDot, wireWikiLinks };
...
/* Утверждения о сущности — то, ради чего вся схема и затевалась.
   ...
   опираться сегодня. */
/* Порядок видов на карточке сущности — по тому, что решает раньше. Сперва то,
   на что опираются в действии (решения, правила, договорённости), потом
   устройство — как оно сейчас собрано, потом сломанное, потом ряды замеров,
   потом остальное. Виды, которых здесь нет, идут следом в порядке схемы: список
   задаёт приоритет, а не полноту, и новый тип не должен из-за него пропадать. */
const CLAIM_ORDER = ['решение', 'правило', 'договорённость', 'намерение',
  'устройство', 'инцидент', 'замер', 'факт', 'наблюдение', 'событие'];
...
export function askTag(path, reload) {
  const wrap = $('#modal');
...
/* Экраны разбора, ленты, людей и архива переехали в ./views/memory.js —
   реэкспорт держит прежние импорты рабочими. */
export { renderTriage, renderTimeline, renderPeople, renderArchive } from './views/memory.js';
