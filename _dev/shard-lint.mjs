...
const ТИПЫ = ['факт', 'решение', 'правило', 'наблюдение', 'договорённость', 'намерение', 'событие'];
...
for (const name of names) {
  ...
  if (!fm['источник']) err(name, 'нет поля «источник» — утверждение без первоисточника непроверяемо');
  else for (const t of wiki(fm['источник'])) if (!resolves(t)) err(name, `источник [[${t}]] не резолвится`);
